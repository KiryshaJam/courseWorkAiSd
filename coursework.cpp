...
void insertionSortByVertices(DynamicArray& arr, int left, int right, const string* vertices) {
    for (int i = left + 1; i <= right; i++) {
        Edge temp = arr[i];
        int j = i - 1;

        // Сравниваем рёбра по алфавиту, если вершины одинаковые, сравниваем веса
        string vertexA1 = vertices[arr[j].u] + vertices[arr[j].v];
        string vertexB1 = vertices[temp.u] + vertices[temp.v];

        while (j >= left && vertexA1 > vertexB1) {
            arr[j + 1] = arr[j];
            j--;
            if (j >= left) {
                vertexA1 = vertices[arr[j].u] + vertices[arr[j].v];
            }
        }
        arr[j + 1] = temp;
    }
...
int kruskalAlgorithm(int n, DynamicArray& edges, const string* vertices) {
    ...
    // Сортируем рёбра по алфавиту (по именам вершин)
    insertionSortByVertices(mst_edges, 0, mst_edges.getSize() - 1, vertices);
...
void DFS(int v, bool* visited, const int graph[MAX_VERTICES][MAX_VERTICES], int n, const string* vertices) {
    visited[v] = true;
    cout << vertices[v] << " ";  // Используем массив вершин для вывода

    for (int i = 0; i < n; ++i) {
        if (graph[v][i] && !visited[i]) {
            DFS(i, visited, graph, n, vertices);  // Передаем массив вершин
        }
    }
}

// Функция для обхода в ширину (BFS)
void BFS(int start, const int graph[MAX_VERTICES][MAX_VERTICES], int n, const string* vertices) {
        ...
        switch (choice) {
            ...
            case 2:
                ...
                if (sub_choice == 1) {
                    ...
                    DFS(0, visited, graph, n, vertices);  // Передаем массив вершин
                    cout << endl;
                } else if (sub_choice == 2) {
                    system("clear");
                    cout << "BFS обход с вершины 0: ";
                    BFS(0, graph, n, vertices);  // Передаем массив вершин
                    cout << endl;
                }
                break;
            case 3:
